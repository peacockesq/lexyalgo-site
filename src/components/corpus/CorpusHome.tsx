...
function toHomeRecord(row: LiveSearchRow): HomeRecord { return { slug: row.slug, route: row.canonical_path || `/corpus/live-authority/?slug=${encodeURIComponent(row.slug)}`, title: row.title, citation: row.citation || row.title, authorityType: row.authority_type, jurisdiction: row.jurisdiction, body: row.body, grade: row.grade }; }
